...
contract ERC721token is ERC721,ERC721Full, Ownable {
    ...
    // name, symbol
    constructor(string memory  tokenName, string memory tokenSymbol,string memory uri,uint _timestamp,uint _mintLimit,bytes32 _updownHash) ERC721Full(tokenName, tokenSymbol) public {
        //
        mintLimit=_mintLimit;
        timestamp=_timestamp;
        _setBaseURI(uri);
        updownHash=_updownHash;
        totalBalance=0;
    }
    function setTransferProxy(address newTransferProxy)public onlyOwnerOrOperator{
        transferProxy=newTransferProxy;
    }
    function mintBatch(address player,uint copies)public onlyOwnerOrOperator{
        //string memory URI = baseURI();
        _tokenIds.increment();
        uint256 first = _tokenIds.current();
        for(uint i=first;i<first+copies;i++){
            _mint(player, i);
            _setTokenURI(i,_baseURI);
            totalBalance+=1;
        }
    }
  ...
  //selfdestruct the contract
    ...
    function getUpdownHash()external view returns(bytes32){
       return updownHash;
    }
...
contract NFToption{
    ...
    mapping(address=>bytes32)public upOrDown;
    mapping(string=>address)public optionAddress;
    //event
    event addGame(uint _startTime,uint _endTime,address indexed _upOption,address indexed _downOption);
    event addUpOption(address indexed newOptionAddress,uint date,uint amountLimit);
    event addDownOption(address indexed newOptionAddress,uint date,uint amountLimit);
    event buyUp(address buyer,uint timestamp,uint amount);
    event buyDown(address buyer,uint timestamp,uint amount);
    event NFTCreated(string tokenName,string tokenSymbol,string tokenURI,uint mintLimit,uint timestamp);
    constructor() public {
    ...
    function buy(address buyer,address to,address optionAddr,uint buyAmount,uint price,uint buyTimestamp)public{
        ...
        uint endStamp=startStamp+86400;
        uint balLimit=option.getMintLimit();
        uint nowtotalBal=option.getTotalBalance();
        ...
        if(updown==keccak256("Up")){
            ...
            emit buyDown(buyer,buyTimestamp,buyAmount);
        }
    }
    /* airdrop NFT copies of work after voting */
    /*
    function airdropNFT(uint _votingId,uint timestamp)public{
        Voting storage voting_=_voting[_votingId];
        require((timestamp>=voting_.endVoteTime)," Event not ended.");
        require(voting_.isAirdropped==false,"Event airdropped!");
        for(uint i=1;i<=voting_.totalParticipant;i++){
            Candidate storage candidate_=_candidate[_votingId][i];
            ...
            for(uint j=0;j<airdroplist.length;j++){
                nft.mintBatch(airdroplist[j],candidate_.URI,1);
            }
        }
    }*/
    ...
    function getBytecode(string memory name,string memory symbol,string memory uri,uint Timestamp,uint mintLimit,bytes32 isupdown)internal pure returns (bytes memory) {
        bytes memory bytecode = type(ERC721token).creationCode;
        return abi.encodePacked(bytecode, abi.encode(name,symbol,uri,Timestamp,mintLimit,isupdown));
    }
    function createNFT(string memory tokenName, string memory tokenSymbol,string memory uri,uint _timestamp,uint _mintLimit,bytes32 isupdown)internal returns (address NFT){
        bytes32 salt=keccak256(abi.encodePacked(tokenName,tokenSymbol,uri,_timestamp,_mintLimit,isupdown));
        bytes memory bytecode=getBytecode(tokenName,tokenSymbol,uri,_timestamp,_mintLimit,isupdown);
        assembly{
        ...
        emit NFTCreated(tokenName,tokenSymbol,uri,_mintLimit,_timestamp);
    }
}
