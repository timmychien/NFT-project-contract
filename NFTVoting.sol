...
contract NFTVoting{
    ...
    function createCandidate(uint _votingId,string memory _NFTName,string memory _NFTSymbol,string memory _URI,string memory _author,address authorAddress,uint timestamp)public returns(address){
        ...
        candidate_.NFTSymbol=_NFTSymbol;
        candidate_.URI=_URI;
    ...
    function vote(uint _votingId,uint participantId,address voter,uint timestamp,bool pointPaid)public{
        Voting storage voting_=_voting[_votingId];
    ...
    function airdropNFT(uint _votingId,uint timestamp)public{
        Voting storage voting_=_voting[_votingId];
        require((timestamp>=voting_.endVoteTime),"This event hasn't been ended yet.");
        require(voting_.isAirdropped==false,"This event has been airdropped!");
    ...
    function getTotalParticipant(uint votingId)public view returns(uint){
        Voting storage voting_=_voting[votingId];
        return voting_.totalParticipant;
    }
    function getnftAddress(uint votingId,uint participantId)public view returns(address){
